...
\bigbreak\noindent%
\Screenshot[window]{Images/Multiple Values.png}
...
Configure these shortcuts in \Path{Glyphs > Preferences… > Shortcuts > Commands}.

\bigbreak\noindent%
\Screenshot[window]{Images/Shortcuts.png}

\bigbreak\noindent\Label{Edit Tags} selects the tags field in the palette.
Press Escape or Return to unselect the field.
\Label{Add Tags}, \Label{Remove Tags}, and \Label{Rename Tags} perform the batch editing operations on all selected glyphs as described in the previous section~(page~\pageref{sec:batch_editing_tags}).

Click \Label{Record shortcut} next to a command name and press a combination of modifier keys~-- Command~(\KeyboardEquivalent{command}), Control~(\KeyboardEquivalent{control}), Shift~(\KeyboardEquivalent{shift}), and Option~(\KeyboardEquivalent{option})~-- and a normal (non-modifier) key.
